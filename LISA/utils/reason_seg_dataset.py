...
from LISA.model_lisa.llava import conversation as conversation_lib
...
from .utils import (ANSWER_LIST, DEFAULT_IMAGE_TOKEN,
                    ...
                    SHORT_QUESTION_LIST)
import sys
sys.path.append('/apdcephfs_qy4/share_302593112/realzliu/code/flow_trans/')
...
class ReasonSegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # sampled_masks = [
        #     ((mask == 1) | (mask == 255)).astype(np.float32)
        #     for _ in range(len(sampled_inds))
        # ]
        ...
        for text in sampled_sents:
            if is_sentence:
                ...
                qq = question_template.format(sent=text)
                questions.append(qq)
            else:
            ...
            if self.explanatory != -1 and img_name in self.img_to_explanation:
                if choice == 0:  # [SEG] token
                    ...
                    questions[-1] = (
                        ...
                        + "\n"
                        + text
                        + " {}".format(random.choice(self.explanatory_question_list)) + ' ' + answer
                    )
                    answers.append("")
