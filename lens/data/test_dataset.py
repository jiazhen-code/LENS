...
import torch
import sys
sys.path.append('/apdcephfs_qy4/share_302593112/realzliu/code/flow_trans/LISA')
from model_lisa.llava import conversation as conversation_lib
...
class TestReferDataset(ReferSegDataset):
    def __init__(
        ...
        SHORT_QUESTION_TEMPLATE = [
            DEFAULT_IMAGE_TOKEN
            + "\n"
            + "What is {class_name} in this image?"
        ]
...
class TestReasoningDataset(ReasonSegDataset):
    ...
    def __getitem__(self, idx):
        ...
        debug = True # set true to align Lisa
        if debug:
        ...
        return (
            ...
            sent_ids,            # sent id (useless now)
            conversation_record,
            is_sentence,         # ReasonSeg: True=long (sentence) query, False=short (phrase)
        )
