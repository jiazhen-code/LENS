...
def collate_fn_val(batch, tokenizer=None, use_mm_start_end=True, padding="right"):
    ...
    for item in batch:
        (image_path, images, images_clip, conversations,
            masks, sam_mask_shape, exists, ref_id, sent_id, conversation_record) = item[:10]
        # TestReasoningDataset appends is_sentence (True=long/sentence, False=short/phrase) as
        # an 11th element; datasets that don't aren't ReasonSeg, so default False is harmless.
        is_sentence = item[10] if len(item) > 10 else False
        image_path_list.append(image_path)
        images_list.append(images)
        images_clip_list.append(images_clip)
        conversation_list.extend(conversations)
    ...
    return {
        ...
        "is_sentence_list": is_sentence_list,
    }
